...
interface AccordionItemProps {
  ...
  function AccordionItem({ item }: AccordionItemProps) {
    ...
    const [data, setData] = useState<DataItem[]>([
        { id: 1, rule: 'Customer claims to have received most of the order?',status:true},
        { id: 2, rule: 'Good track record: - 30% rate of return + no refund request + 100€?',status:true },
        { id: 4, rule:  'Package damaged according to the customer?',status:true  },
        { id: 3, rule: 'Many packages lost in the same sector recently?', status:true},
        { id: 5, rule: 'Package lost/returned contain a luxury items - price higher than 200€?', status:true },
...
export default function CustomerRule({ className }: { className?: string }) {
