...
import { Accordion, Button, Progressbar, Text } from 'rizzui';
...
interface AccordionItemProps {
  ...
  function AccordionItem({ item }: AccordionItemProps) {
    ...
    const [data, setData] = useState<DataItem[]>([
        { id: 1, rule: 'User have suspicious ports opened on his IP',status:true },
        { id: 2, rule: 'User is using a public proxy',status:"+2" },
        { id: 4, rule: 'User is using a Data-center proxy',status:"+7"  },
        { id: 3, rule: 'User is using a web proxy',status:"+6" },
        { id: 5, rule: 'USER is using TOR', status:"+54" },
        { id: 6, rule: 'IP blacklisted',status:true  },
        { id: 7, rule: 'USER is using a HARMFUL Ip address',status:"+2"  },
      ]);
...
export default function IPRule({ className }: { className?: string }) {
