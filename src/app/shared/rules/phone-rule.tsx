...
interface AccordionItemProps {
  ...
  function AccordionItem({ item }: AccordionItemProps) {
    ...
    const [data, setData] = useState<DataItem[]>([
        { id: 1, rule:  'This phone is not valid',status:true},
        { id: 2, rule: 'This phone is a VOIP phone number',status:true },
        { id: 4, rule:  'This phone is disposable',status:true  },
    ...
    return (
      ...
      >
<Accordion.Header>
        <div className="flex w-full cursor-pointer items-center justify-between py-5 text-l font-semibold">
          <div className="flex w-full items-start justify-between w-1/3">{item.rule}</div>
          <div className="w-1/3 ml-2  text-green-500 dark:text-green-400">
...
export default function PhoneRule({ className }: { className?: string }) {
