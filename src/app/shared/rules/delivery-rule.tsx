...
interface AccordionItemProps {
  ...
  function AccordionItem({ item }: AccordionItemProps) {
    ...
    const [data, setData] = useState<DataItem[]>([
        { id: 1, rule: 'Delivery happen as planned, no incident?',status:true},
        { id: 2, rule: 'Returned packaged match the weight of the items returned?',status:true },
        { id: 4, rule: 'Package damaged according to the delivery company?',status:true  },
        { id: 3, rule: 'Many packages lost in the same sector recently?', status:true},
        { id: 5, rule: 'Delivery company perform a structural change?', status:true },
        { id: 6, rule: 'Many incidents recently with this delivery company?', status:true},
      ]);
...
export default function DeliveryRule({ className }: { className?: string }) {
